...
The dashboard can also fetch live data from the Cloudflare Worker every 60 seconds. Set the GitHub Repository Secret `DASHBOARD_STATS_URL` to the deployed Worker endpoint, for example `https://translation-review-dashboard-api.<subdomain>.workers.dev/api/dashboard-stats`. If that secret is absent, the app falls back to the static `dashboard-stats.json` generated by GitHub Actions.
...
The `worker` folder serves live dashboard data from NoCoDB without exposing the NoCoDB token to the browser. GitHub Repository Secrets are available only to GitHub Actions; they are not available inside Cloudflare Workers. Set Cloudflare Worker secrets and variables separately, especially:
